...
2. **Evaluation (separate clean environment).** Pull the base and fine-tuned
   models from the Hub and run bigcode-evaluation-harness on the held-out APPS
   test split, executing generated code against hidden tests to compute pass@1 per
   difficulty tier.
...
code was executed against APPS' hidden test cases by the harness. The reported
**pass@1 is strict accuracy** — a problem counts as solved only if every hidden
test passes. A softer secondary metric, the average fraction of individual test
cases passed, is used for additional resolution.
...
On the medium (interview) tier — the only tier where either model solved any
problem outright — the base model reached 7.3 percent pass@1 while the fine-tune
reached 2.0 percent. Both models scored 0 percent on the easy and hard tiers at
this sample size, which is why the second metric is informative.
...
With partial credit the gap is visible on every tier, and the base leads
throughout: 30.2 versus 7.4 percent on medium, 0.84 versus 0.22 percent on easy,
and 0.54 versus 0.0 percent on hard. In other words, the fine-tune is not merely
failing to fully solve problems — it is getting fewer individual test cases right
across the board.
...
- **Data selection:** stop training on the shortest solution; use cleaner,
  medium-length or multiple solutions per problem so the model learns good code
  rather than golfed code.
- **Reduce forgetting:** lower the learning rate and/or number of steps, and
  consider mixing in general instruction data.
- **Broader evaluation:** re-run on the same 150 problems per tier for a direct
  v1-to-v2 comparison, and add a LiveCodeBench evaluation plus a published
  frontier column for external context. LiveCodeBench is never used for training.
- **Scale:** evaluate on a larger subset (or the full test set) once generation
  throughput allows.
