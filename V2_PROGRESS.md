...
- [ ] **Re-run `--show-io` with fixed loader** (question must now match code). Then read the true base pass rate on aligned introductory: if code matches question and STILL fails → base failure is real; if it PASSES → the v1 harness had its own misalignment (measurement bug) and v1's 0% is invalid.
